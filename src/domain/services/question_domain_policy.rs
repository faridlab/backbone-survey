//! Domain policy for Question aggregate
//!
...
/// Domain policy for Question — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Question>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type QuestionDomainPolicy = PermitAllPolicy<Question>;
